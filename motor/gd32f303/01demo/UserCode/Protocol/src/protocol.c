  ...
  while(1)
  {
    ...
    switch (cmd_type)
    {
      ...
      case SET_P_I_D_CMD:
      {
        ...
        // utils_pid_set_pid_value(&current_pid, p_temp, i_temp, d_temp);
        ...
        {
          utils_pid_set_pid_value(&speed_pid, p_temp, i_temp, d_temp);
        }
      }
      break;
      ...
      case SET_TARGET_CMD:
      {
        ...
        // device_motor_set_direction(MOTOR_REV);
		    // device_motor_set_speed(actual_temp);
        // device_motor_enable();
        /* 只设置速度的目标值，电流的目标置是由速度pid的输出决定的 */
        if (packet.ch == CURVES_CH1)
        {
          utils_pid_set_target_value(&current_pid, actual_temp);
        }
        if (packet.ch == CURVES_CH2)
        {
          utils_pid_set_target_value(&speed_pid, actual_temp);
        }
      }
      break;
      ...
      case START_CMD:
      {
        device_motor_enable();              // 启动电机
      }
      break;

      case STOP_CMD:
      {
        device_motor_disable();              // 停止电机
      }
      break;
