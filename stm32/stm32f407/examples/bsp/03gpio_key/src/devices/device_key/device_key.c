...
static uint8_t key_pin_active(uint8_t _id) {
    ...
    if ((s_gpio_list[_id].gpiox->IDR & s_gpio_list[_id].gpio_pin) == 0) {
        level = 0;
    } else {
        level = 1;
    }

    if (level == s_gpio_list[_id].active_level) {
        return 1;
    } else {
        return 0;
    }
...
int key_device_init(void)
{
    ...
    for (int i = 0; i < ARRAY_SIZE(s_gpio_list); i++)
    {
        device_key_hard_init(&s_gpio_list[i]);
    }
    return 0;
}
INIT_BOARD_EXPORT(key_device_init);
